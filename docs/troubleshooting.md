...
See [mcp-integration.md](mcp-integration.md#absolute-paths-for-envfile-distrobox--containers).
...
The `PYTHONPATH` env var pointing at `src/` is a belt-and-suspenders fallback
for environments that override `PYTHONHOME`.
...
If using a separate `TEMPO_API_TOKEN`, rotate that too.
...
See [reports.md](reports.md#path-traversal-protection).
