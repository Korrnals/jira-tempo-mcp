...
| Переменная | Обяз. | По умолч. | Описание |
| --- | --- | --- | --- |
| `REPORT_OUTPUT_DIR` | нет | `~/.mcp/jira-tempo-mcp/reports/` | Базовая директория для файлов отчётов; вложенные `год/месяц/weekly/` создаются молчаливо |
...
### 🛠️ Источник dotenv (опциональные)

| Переменная | Обяз. | По умолч. | Описание |
| --- | --- | --- | --- |
| `MCP_ENV_FILE` | нет | пусто | Явный абсолютный путь к `.env.local`; переопределяет стандартные расположения VS Code |

---

## 🔗 Приоритет источников конфигурации

Сервер собирает конфигурацию из нескольких источников. Приоритет (от
высшего к низшему) определяет, какое значение выигрывает при конфликте:

| # | Источник | Когда применяется |
| --- | --- | --- |
| 1 | **Переменные окружения процесса** | Всегда выигрывают — задаёт MCP-host, systemd, CI/CD или shell |
| 2 | **MCP-host `.env.local`** | Первый существующий файл: `MCP_ENV_FILE` → `~/.config/Code/User/.env.local` (Linux) → `~/Library/Application Support/Code/User/.env.local` (macOS) → `%APPDATA%/Code/User/.env.local` (Windows) |
| 3 | **`.env` в корне репо** | Файл `.env` рядом с `pyproject.toml`; удобен для CLI-разработки |
| 4 | **Значения по умолчанию** | Жёстко заданы в `config.py` (напр. `~/.mcp/jira-tempo-mcp/reports/`) |

> 💡 **Как это работает:** все dotenv-файлы загружаются через
> `load_dotenv(override=False)` — уже заданные переменные процесса **не
> перезаписываются**. Файлы применяются в порядке `.env.local` → репо-`.env`,
> поэтому для ключей, отсутствующих в окружении процесса, выигрывает первый
> загрузивший файл (`.env.local`).

> ⚠️ **Сценарий, который это решает:** при прямом вызове Python из терминала
> (вне MCP-host) переменные из `.env.local` **раньше не подхватывались** —
> отчёты падали в default-директорию, требуя ручного копирования. Теперь
> терминальный вызов читает тот же `.env.local`, что и MCP-сервер.
