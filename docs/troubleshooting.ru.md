...
См. [mcp-integration.ru.md](mcp-integration.ru.md#абсолютные-пути-для-envfile-distrobox--контейнеры).
...
Переменная `PYTHONPATH` на `src/` — дополнительный фолбэк для окружений,
переопределяющих `PYTHONHOME`.
...
Если используется отдельный `TEMPO_API_TOKEN`, перевыпустите и его.
...
См. [reports.ru.md](reports.ru.md#защита-от-path-traversal).
...
## Сервер стартует, но инструменты уходят в таймаут
