...
Три MCP-инструмента покрывают обнаружение, предпросмотр и генерацию шаблонов:

| Инструмент | Назначение | Вызывает Jira/Tempo? |
| --- | --- | --- |
| `list_report_templates` | Список всех доступных шаблонов (встроенные + пользовательские), с типом движка (`Jinja2`/`Python`) и источником (`builtin`/`custom`) | Нет |
| `preview_report_template` | Рендер шаблона на встроенных тестовых worklog'ах; просмотр вывода без создания файла | Нет |
| `generate_weekly_report` | Генерация реального отчёта с выбором шаблона через параметр `template` | Да |

### `preview_report_template` — изучить перед генерацией

Предпросмотр рендерит шаблон на встроенных примерах worklog'ов, чтобы можно
было доводить вёрстку, не тратя вызов Jira/Tempo. Возвращает отрендеренный
текст строкой и ничего не записывает на диск.

| Параметр | Обязателен | Описание |
| --- | --- | --- |
| `template_name` | ✅ | Имя шаблона для предпросмотра (имя файла `.j2` без расширения, например `standup`). Допустимые имена — через `list_report_templates`. |
| `sample_data` | — | Профиль демонстрационных данных. **По умолчанию:** `default`. |

Профили `sample_data`:

| Значение | Используемые worklog'и |
| --- | --- |
| `default` | Несколько реалистичных worklog'ов с разным временем |
...
> 💡 **Рабочий процесс:** `list_report_templates` → `preview_report_template`
> (настроить вёрстку на тестовых данных) → `generate_weekly_report(template="standup")`
> (получить реальный отчёт). Полный справочник для авторов — в
> [templates.ru.md](templates.ru.md).
